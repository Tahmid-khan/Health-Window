...
exports.registerDoctor = async (req, res) => {
  ...
  try {
    ...
    let specialtyId;
    const [specialtyRows] = await conn.query("SELECT specialty_id FROM specialty WHERE name = ?", [specialty]);

    if (specialtyRows.length > 0) {
      specialtyId = specialtyRows[0].specialty_id;
    } else {
      const [specialtyInsert] = await conn.query("INSERT INTO specialty (name) VALUES (?)", [specialty]);
      specialtyId = specialtyInsert.insertId;
    }

    await conn.query(
      `INSERT INTO doctor (user_id, medical_license_number, qualification, experience_years, specialty, bio)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [userId, license, qualification, experience, specialtyId, bio]
    );
  ...
  try {
    const [rows] = await db.query(`
      SELECT 
        ...
        s.name AS specialty,
        d.qualification, d.experience_years AS experience,
        d.bio, d.clinic_name, d.address
      FROM user u
...
// Update Doctor Profile (clinic name and address)
exports.updateDoctorProfile = async (req, res) => {
  const { user_id, clinic_name, address } = req.body;
  ...
  try {
    await db.query(
      `UPDATE doctor SET clinic_name = ?, address = ? WHERE user_id = ?`,
      [clinic_name, address, user_id]
    );
